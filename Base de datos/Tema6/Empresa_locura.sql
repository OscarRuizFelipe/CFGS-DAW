...
/*Crea el procedimiento ac12mediaSalarial que, a partir de un código de departamento (parámetro de entrada), devuelva en un parámetro de salida el
 salario medio de los empleados de dicho departamento. usa la base de datos empresa de arriba*/

DELIMITER //
CREATE PROCEDURE ac12mediaSalarial(IN codDep CHAR(5), OUT mediaSalarial DECIMAL(12,2))
BEGIN
   SELECT AVG(SalEmp) INTO mediaSalarial
   FROM empleado
   WHERE CodDep = codDep;
END //
...
CREATE FUNCTION ac12categoriaDepartamento(codDep CHAR(5))
RETURNS VARCHAR(10)

BEGIN
   DECLARE mediaSalarial DECIMAL(12,2);
   DECLARE categoria VARCHAR(10);

   CALL ac12mediaSalarial(codDep, mediaSalarial);

   IF mediaSalarial < 2000000 THEN
      SET categoria = 'bajo';
   ELSEIF mediaSalarial <= 5000000 THEN
      SET categoria = 'medio';
   ELSE
      SET categoria = 'alto';
   END IF;
...
/*Crea el procedimiento ac12actualizaInforme que, a partir de un código de departamento (parámetro de entrada), actualice la fila correspondiente en informe_salarial 
con la cantidad de empleados, el salario medio y la categoría salarial. Este procedimiento debe invocar a ac12mediaSalarial y a ac12categoriaDepartamento.*/
DELIMITER //

CREATE PROCEDURE ac12actualizaInforme(IN codDep CHAR(5))    
BEGIN
   DECLARE mediaSalarial DECIMAL(12,2);
   DECLARE categoria VARCHAR(10);
   ...
   -- Comprobar si ya existe en informe_salarial
   SELECT COUNT(*) INTO existe
   FROM informe_salarial
   WHERE CodDep = codDep;
   ...
   ELSE
      ...
      (CodDep, NomDep, NumEmpleados, SalarioMedio, Categoria)
      VALUES (codDep, nomDep, numEmpleados, mediaSalarial, categoria);
   END IF;
